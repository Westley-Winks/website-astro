...
_This piece first appeared on the [Until It’s Not Fun](https://untilitsnotfun.com/posts/2022-06-03/) newsletter._

The internet. We all know it. We all love it.

But what actually _is_ it?
...
The DNS provides a map between a domain name (untilitsnotfun.com) to a more accurate server location called an IP address. It's like you typing in "Cafe Yumm" into Google Maps and it returning the very precise latitude and longitude coordinates. You make a request to the DNS that you want to see untilitsnotfun.com and it points that request to the actual server where the untilitsnotfun.com _files_ are. The server then hears the request and responds.
...
Come back next week for part two where we dive into a huge building block of the internet: [APIs](/writing/apis/).
